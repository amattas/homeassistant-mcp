...
class HomeAssistantService:
    ...
    def __init__(self, url: str, access_token: str, verify_ssl: bool = True):
        ...
        logger.info(
            f"Initialized Home Assistant service ({self.connection_type.value}): {self.url}"
        )
    ...
    def _validate_temperature(
        self, temperature: Optional[float], unit: str = "C"
    ) -> None:
    ...
    def get_states(
        ...
        if area:
            ...
            if area_ids:
                ...
                area_entity_ids = [
                    e["entity_id"] for e in entities if e.get("area_id") in area_ids
                ]
        ...
        if response_size > 900000:  # ~900KB
            ...
            logger.warning(
                f"Response too large ({response_size} bytes), truncating to 100 states"
            )
    ...
    def set_value(self, entity_id: str, value: Any) -> Dict[str, Any]:
        ...
        if domain == "input_number":
            return self.call_service(
                domain, "set_value", entity_id=entity_id, value=float(value)
            )
            ...
            return self.call_service(
                domain, "select_option", entity_id=entity_id, option=str(value)
            )
            ...
            if isinstance(value, str):
                return self.call_service(
                    domain, "set_datetime", entity_id=entity_id, datetime=value
                )
            elif isinstance(value, dict):
                return self.call_service(
                    domain, "set_datetime", entity_id=entity_id, **value
                )
        else:
            return {
                "status": "error",
                "error": f"Unsupported domain for set_value: {domain}",
            }
    ...
    def run_script(self, script_id: str, **variables) -> Dict[str, Any]:
        ...
        return self.call_service(
            "script", "turn_on", entity_id=script_id, variables=variables
        )
    ...
    def _get_areas_via_websocket(self) -> Optional[List[Dict[str, Any]]]:
        ...
        if websocket is None:
            logger.warning(
                "WebSocket support not available - websocket-client not installed"
            )
            return None
        try:
            ...
            ws = websocket.create_connection(
                ws_url, sslopt=sslopt, timeout=self.timeout
            )
            ...
            try:
                ...
                # Send authentication
                auth_msg = {"type": "auth", "access_token": self.access_token}
                ...
                area_request = {"id": 1, "type": "config/area_registry/list"}
                ...
                if data.get("success"):
                    ...
                    logger.info(
                        f"Successfully retrieved {len(areas)} areas via WebSocket"
                    )
                    return areas
                else:
                    error = data.get("error", {})
                    logger.error(
                        f"WebSocket area request failed: {error.get('message', 'Unknown error')}"
                    )
                    return None
    ...
    def get_devices(
        self, minimal: bool = True, limit: Optional[int] = None, offset: int = 0
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                if not devices:
                    logger.info(
                        "Device registry not available through Nabu Casa, and no device info in states"
                    )
                    return []
    ...
    def get_entities(
                ...
                return [
                    {
                        ...
                        "device_class": e.get(
                            "device_class"
                        ),  # Keep device_class as it's useful for understanding entity type
                    }
                    for e in entities
                ]
            return entities
        ...
        for state in states:
            ...
            entity_info = {
                ...
                "domain": (
                    state.get("entity_id", "").split(".")[0]
                    if "." in state.get("entity_id", "")
                    else "unknown"
                ),
...
• Occupancy-based control""",
    ...
    def set_light_level_for_mcp(
        self, entity_id: str, brightness_percent: str
    ) -> Dict[str, Any]:
    ...
    def set_climate_for_mcp(
        ...
        try:
            ...
            if temperature is not None:
                ...
                return self.call_service(
                    "climate", "set_preset_mode", entity_id, preset_mode=preset_mode
                )
            else:
    ...
    def control_cover_for_mcp(
        self, entity_id: str, action: str, position: Optional[Union[int, str]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if action == "open":
                ...
                return {
                    "error": f"Invalid action '{action}' or missing position for set_position"
                }
        except Exception as e:
            return {"error": str(e), "entity_id": entity_id}

    def lock_control_for_mcp(
        self, entity_id: str, action: str, code: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def run_script_for_mcp(
        self, script_id: str, variables: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """MCP wrapper for run_script"""
        return self.run_script(script_id, **(variables or {}))

    def control_media_for_mcp(
        self, entity_id: str, action: str, volume: Optional[Union[float, str]] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if action == "play":
                ...
                return self.call_service(
                    "media_player", "volume_mute", entity_id, is_volume_muted=True
                )
            elif action == "volume_unmute":
                return self.call_service(
                    "media_player", "volume_mute", entity_id, is_volume_muted=False
                )
            else:
                return {
                    "error": f"Invalid action '{action}' or missing volume for volume_set"
                }
        except Exception as e:
            return {"error": str(e), "entity_id": entity_id}

    def control_area_for_mcp(
        self,
        area_name: str,
        action: str,
        domain: str = "light",
        ...
        states = self.get_states(
            area=area_name, domain=domain if domain != "all" else None
        )
        ...
        for entity_id in entity_ids:
            if action == "turn_on":
                r = self.call_service(
                    entity_id.split(".")[0], "turn_on", entity_id, **kwargs
                )
            elif action == "turn_off":
                r = self.call_service(
                    entity_id.split(".")[0], "turn_off", entity_id, **kwargs
                )
            elif action == "toggle":
                r = self.call_service(
                    entity_id.split(".")[0], "toggle", entity_id, **kwargs
                )
            results.append(r)
        ...
        return {
            "status": "success",
            "area": area_name,
            "entities_controlled": entity_ids,
            "results": results,
        }

    def send_notification_for_mcp(
        self, message: str, title: Optional[str] = None, target: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def get_states_by_domain_for_mcp(
        self, domain: str, area: Optional[str] = None
    ) -> Dict[str, Any]:
        """Get states filtered by domain and optionally by area"""
        return self.get_states(domain=domain, area=area)

    def get_states_by_area_for_mcp(
        self, area: str, domain: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def get_states_paginated_for_mcp(
        self,
    ...
    def get_devices_paginated_for_mcp(
        self,
        minimal: Optional[Union[bool, str]] = None,
        limit: Optional[Union[int, str]] = None,
        offset: Optional[Union[int, str]] = None,
    ) -> List[Dict[str, Any]]:
    ...
    def get_entities_paginated_for_mcp(
        self,
        minimal: Optional[Union[bool, str]] = None,
        limit: Optional[Union[int, str]] = None,
        offset: Optional[Union[int, str]] = None,
    ...
    def get_entity_history_resource(
        self, entity_id: str, hours: Union[int, str] = 24
    ) -> Dict[str, Any]:
    ...
    def get_scripts_resource(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"scripts": scripts, "script_count": len(scripts)}
        except Exception as e:
    ...
    def get_sensors_by_type_resource(self, sensor_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for sensor in sensors:
                ...
                if (
                    sensor_type.lower() in entity_id.lower()
                    or sensor_type.lower() == device_class.lower()
                ):
    ...
    def get_unavailable_entities_resource(self) -> Dict[str, Any]:
        ...
        try:
            ...
            unavailable = [
                s for s in all_states if s.get("state") in ["unavailable", "unknown"]
            ]
    ...
    def categorize_sensors(self) -> Dict[str, Any]:
        ...
        try:
            from helpers.sensor_categorizer import SensorCategorizer
    ...
    def get_sensors_by_category(self, category: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"category": category, "sensors": detailed, "count": len(detailed)}
        except Exception as e:
            ...
            "domains": [
                {
                    ...
                    "common_attributes": ["brightness", "color_temp", "rgb_color"],
                },
                {
                    "domain": "switch",
                    "description": "On/off switches",
                    "common_attributes": ["power", "current"],
                },
                {
                    "domain": "sensor",
                    "description": "Sensor readings",
                    "common_attributes": ["unit_of_measurement", "device_class"],
                },
                {
                    "domain": "binary_sensor",
                    "description": "Binary sensors (on/off)",
                    "common_attributes": ["device_class"],
                },
                {
                    "domain": "climate",
                    "description": "Thermostats and HVAC",
                    "common_attributes": [
                        "temperature",
                        "target_temp_high",
                        "target_temp_low",
                ...
                {
                    "domain": "cover",
                    "description": "Covers, blinds, garage doors",
                    "common_attributes": ["position", "tilt_position"],
                },
                {
                    "domain": "lock",
                    "description": "Smart locks",
                    "common_attributes": ["locked"],
                },
                {
                    "domain": "media_player",
                    "description": "Media devices",
                    "common_attributes": ["volume_level", "source", "media_title"],
                },
                {
                    "domain": "fan",
                    "description": "Fans",
                    "common_attributes": ["speed", "oscillating", "direction"],
                },
                {
                    "domain": "vacuum",
                    "description": "Robot vacuums",
                    "common_attributes": ["battery_level", "status"],
                },
                {
                    "domain": "camera",
                    "description": "Cameras",
                    "common_attributes": ["video_url", "still_image_url"],
                },
                {"domain": "scene", "description": "Scenes", "common_attributes": []},
                {
                    "domain": "automation",
                    "description": "Automations",
                    "common_attributes": ["last_triggered"],
                },
                {
                    "domain": "script",
                    "description": "Scripts",
                ...
                },
                {
                    ...
                    "description": "Boolean helpers",
                    "common_attributes": [],
                },
                {
                    "domain": "input_number",
                    "description": "Number helpers",
                    "common_attributes": ["min", "max", "step"],
                },
                {
                    "domain": "input_text",
                    "description": "Text helpers",
                    "common_attributes": ["min", "max", "pattern"],
                },
                {
                    "domain": "input_select",
                    "description": "Dropdown helpers",
                    "common_attributes": ["options"],
                },
                {
                    "domain": "timer",
                    "description": "Timer helpers",
                    "common_attributes": ["duration", "remaining"],
                },
                {
                    "domain": "counter",
                    "description": "Counter helpers",
                    "common_attributes": ["minimum", "maximum", "step"],
                },
            ],
    ...
    def get_lights_on_resource(self) -> Dict[str, Any]:
        ...
        lights_on = [light for light in lights if light.get("state") == "on"]
    ...
    def get_temperature_sensors_resource(self) -> Dict[str, Any]:
        ...
        for sensor in sensors:
            ...
            if attrs.get("device_class") == "temperature" or attrs.get(
                "unit_of_measurement"
            ) in ["°C", "°F", "celsius", "fahrenheit"]:
                temp_sensors.append(
                    {
    ...
    def get_door_window_sensors_resource(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_type": {
                ...
                "garage_doors": [
                    s for s in door_window_sensors if s["type"] == "garage_door"
                ],
            },
        }
    ...
    def get_security_status_resource(self) -> Dict[str, Any]:
                ...
                security_info["alarms"].append(
                    {
                        ...
                        "armed": entity.get("state")
                        not in ["disarmed", "unknown", "unavailable"],
                    }
                )
            elif domain == "camera":
        ...
        unlocked_locks = [lock for lock in security_info["locks"] if not lock["locked"]]
        ...
        return {
            ...
            "summary": {
                ...
                "motion_detected": motion_detected,
                "secure": len(unlocked_locks) == 0
                and len(open_doors) == 0
                and len(open_windows) == 0,
            },
        }
    ...
    def get_battery_status_resource(self) -> Dict[str, Any]:
        ...
        for entity in all_states:
            ...
            # Also check for battery sensors
                ...
                and attrs.get("device_class") == "battery"
            ):
                try:
                    ...
                    logger.debug(
                        "Failed to parse battery level for %s: %s",
    ...
    def _get_entity_area(self, entity_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.debug("Failed to resolve area for entity %s: %s", entity_id, exc)
        return None
