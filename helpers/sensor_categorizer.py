...
class SensorCategorizer:
    ...
    def compile_patterns(self):
        ...
        self.weather_regex = [
            re.compile(p, re.IGNORECASE) for p in self.WEATHER_PATTERNS
        ]
        self.pool_regex = [re.compile(p, re.IGNORECASE) for p in self.POOL_PATTERNS]
        self.air_quality_regex = [
            re.compile(p, re.IGNORECASE) for p in self.AIR_QUALITY_PATTERNS
        ]
        self.hvac_regex = [re.compile(p, re.IGNORECASE) for p in self.HVAC_PATTERNS]
        self.indoor_temp_regex = [
            re.compile(p, re.IGNORECASE) for p in self.INDOOR_TEMP_PATTERNS
        ]
    ...
    def categorize_sensor(self, entity: Dict[str, Any]) -> SensorCategory:
        ...
        # Check weather
        if (
            self._matches_patterns(search_text, self.weather_regex)
            or device_class == "weather"
        ):
        ...
        if device_class in ["temperature", "humidity"]:
            ...
            if any(
                word in search_text
                for word in ["outdoor", "outside", "exterior", "garden", "yard"]
            ):
        ...
        # Energy sensors
        if (
            device_class in ["power", "energy", "voltage", "current"]
            or "kwh" in search_text
            or "watt" in search_text
        ):
            return SensorCategory.ENERGY

        # Security sensors
        if device_class in ["motion", "door", "window", "lock"] or any(
            word in search_text
            for word in ["motion", "door", "window", "lock", "camera"]
        ):
    ...
    def categorize_sensors(
        ...
        for entity in entities:
            ...
            entity_id = entity.get("entity_id", "")
            if not entity_id.startswith("sensor.") and not entity_id.startswith(
                "weather."
            ):
                continue
        ...
        for category, entities in categorized.items():
            if entities:  # Only include non-empty categories
                summary["categories"][category] = {
                    "count": len(entities),
                    "percentage": round(
                        len(entities) / summary["total_sensors"] * 100, 1
                    ),
    ...
    def filter_by_categories(
        self, entities: List[Dict[str, Any]], categories: List[str]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_recommendations(
        ...
        if not categorized.get("weather"):
            recommendations.append(
                "Consider adding weather sensors or integrating a weather service"
            )

        if not categorized.get("indoor_air_quality"):
            recommendations.append(
                "Consider adding indoor air quality sensors (CO2, VOC, PM2.5)"
            )
        ...
        if total > 0:
            ...
            if hvac_percent > 40:
                recommendations.append(
                    "You have many HVAC sensors - consider grouping them by zone"
                )

        # Pool safety
        if categorized.get("pool") and len(categorized["pool"]) < 3:
            recommendations.append(
                "Consider adding more pool sensors for safety (pH, chlorine, temperature)"
            )
