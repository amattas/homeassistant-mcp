...
Examples:
  ...
  python verify_auth.py --api-key test-key --md5-salt test-salt --domain localhost:8080 --no-https
    ...
    parser.add_argument(
        "--no-https", help="Use HTTP instead of HTTPS", action="store_true"
    )
    ...
    if not args.api_key:
        parser.print_help()
        print(
            "\n❌ Error: API key required (use --api-key or set MCP_API_KEY)",
            file=sys.stderr,
        )
        sys.exit(1)

    # Calculate URLs; redact secrets unless explicitly requested otherwise
    result = calculate_mcp_url(
        args.api_key, args.domain, not args.no_https, args.md5_salt
    )
    ...
    if args.json:
        ...
        print(
            f"MD5 Salt:        {'Yes (configured)' if output['md5_salt_used'] else 'No (using legacy mode)'}"
        )
        ...
        if args.show_secrets:
            print(
                "⚠️  Keep the MCP URL confidential. It contains authentication credentials."
            )
        else:
            print(
                "🔒  Secrets are redacted. Re-run with --show-secrets to print the full URL."
            )
        if not result["md5_salt_used"]:
            print(
                "⚠️  Consider setting MD5_SALT environment variable for enhanced security."
            )
        print()
