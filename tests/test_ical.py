...
class TestCalendarFeed:
    ...
    def test_init_with_name(self):
        ...
        feed = CalendarFeed(
            url="https://example.com/calendar.ics", name="Work Calendar"
        )
...
class TestMultiCalendarService:
    ...
    @pytest.mark.parametrize(
        "date_str", ["12/31/2024", "2024-13-01", "invalid", "31-12-2024"]
    )
    ...
    def test_add_feed(self):
        ...
        with patch.object(service, "_refresh_single_calendar") as mock_refresh:
            ...
            result = service.add_feed(
                url="https://example.com/new.ics", name="New Calendar"
            )
    ...
    def test_get_events_single_feed(self, mock_recurring):
        ...
        events = service.get_events(start_date="2024-01-01", end_date="2024-01-31")
    ...
    def test_get_events_multiple_feeds(self):
        ...
        for i, name in enumerate(["Personal", "Work"]):
            ...
            event.add(
                "dtstart", datetime(2024, 1, i + 1, 10, 0, 0, tzinfo=timezone.utc)
            )
        ...
        with patch("recurring_ical_events.of") as mock_recurring:
            ...
            mock_recurring.side_effect = [
                [list(cal.walk("VEVENT"))[0]]
                for cal in [f.calendar for f in service.feeds.values()]
            ]
    ...
    def test_get_events_with_pagination(self, mock_recurring):
        ...
        events = service.get_events(
            start_date="2024-01-01", end_date="2024-01-31", limit=3, offset=0
        )
        assert len(events) == 3
        ...
        # Get next 3 events
        events = service.get_events(
            start_date="2024-01-01", end_date="2024-01-31", limit=3, offset=3
        )
        assert len(events) == 3
        ...
        events = service.get_events(
            start_date="2024-01-01", end_date="2024-01-31", offset=7
        )
        assert len(events) == 3
